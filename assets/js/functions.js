...
(function($){
	...
	var top_icon = {
		start: function(){
			TweenLite.from($(".top_qrcode"), 1, {top: -2, ease: Elastic.easeOut});
		},
		loop: function(){
	...
	var addsList = {
		...
		start: function(){
				...
				for(j in addsList.data[i].data){
					...
					if(addsList.data[i].data[j].adds.toString() == iplookup.city.toString()){
						...
						if(addsList.data[i].data[j].adds.indexOf('上海') > -1)
						{
							...
							$('.select_left_all').html('<a target="_blank" href="http://creditcard.pingan.com/cms-tmplt/creditecard/searchPreferentialInformDetail.do?wid=40001887"><li>中石油</li></a>'
							+'<a target="_blank" href="http://creditcard.pingan.com/cms-tmplt/creditecard/searchPreferentialInformDetail.do?wid=40001888"><li>中石化</li></a>')
						...
						}else if(addsList.data[i].data[j].adds.indexOf('北京') > -1)
						{
							...
							$('.select_left_all').html('<a target="_blank" href="http://creditcard.pingan.com/cms-tmplt/creditecard/searchPreferentialInformDetail.do?wid=40001897"><li>中石油</li></a>'+
							'<a target="_blank" href="http://creditcard.pingan.com/cms-tmplt/creditecard/searchPreferentialInformDetail.do?wid=40001898"><li>中石化</li></a>')
			...
			$('.select_left_btn').bind('click' , function (){
